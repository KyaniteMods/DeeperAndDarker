...
public class DDBlockLootTableProvider extends FabricBlockLootTableProvider {
    ...
    public void generate() {
        ...
        add(DDBlocks.GLEAM_GEL_BLOCK, createSilkTouchDispatchTable(DDBlocks.GLEAM_GEL_BLOCK, this.applyExplosionDecay(DDBlocks.GLEAM_GEL_BLOCK, LootItem.lootTableItem(DDItems.GLEAM_GEL).apply(SetItemCountFunction.setCount(UniformGenerator.between(0.0f, 1.0f))))));
        ...
        return LootTable.lootTable().withPool(
                LootPool.lootPool().setRolls(ConstantValue.exactly(1.0f))
                        .add(LootItem.lootTableItem(DDItems.ICE_LILY)
                                .apply(SetNbtFunction.setTag(noFlower).when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(block).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(IceLilyBlock.HAS_FLOWER, false))))
                        ));
    }
}