    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pContext) {
        ...
        if(level.isEmptyBlock(origin)) {
            ...
            if(state.is(Blocks.SCULK) || state.is(DDBlocks.SCULK_STONE)) {
                level.setBlock(origin, DDBlocks.SCULK_GLEAM.defaultBlockState(), 3);
                ...
                for(int i = 0; i < 1500; ++i) {
                    ...
                    if(level.getBlockState(pos).isAir()) {
                        ...
                        for(Direction direction : Direction.values()) {
                            if(level.getBlockState(pos.relative(direction)).is(DDBlocks.SCULK_GLEAM)) j++;
                            if(j > 1) break;
                        }

                        if(j == 1) level.setBlock(pos, DDBlocks.SCULK_GLEAM.defaultBlockState(), 3);
                    }
                }
