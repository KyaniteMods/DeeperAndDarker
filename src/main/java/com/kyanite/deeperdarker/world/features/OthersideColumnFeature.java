...
public class OthersideColumnFeature extends Feature<ColumnFeatureConfiguration> {
    public OthersideColumnFeature(Codec<ColumnFeatureConfiguration> pCodec) {
        super(pCodec);
    }

    @Override
    public boolean place(FeaturePlaceContext<ColumnFeatureConfiguration> pContext) {
        ColumnFeatureConfiguration config = pContext.config();
        WorldGenLevel level = pContext.level();
        BlockPos origin = pContext.origin();
        RandomSource random = pContext.random();if(!level.getBlockState(origin).isAir()) return false;Optional<Column> scan = Column.scan(level, origin, 64, BlockBehaviour.BlockStateBase::isAir, blockState -> !blockState.isAir());
    ...
    private void placeSection(WorldGenLevel level, RandomSource random, BlockPos pos, int baseHeight, int iteration, int loop, double multiplier, boolean bottom) {
        ...
        for(int j = 0; j < baseHeight; j++) {
            ...
            else level.setBlock(location, DDBlocks.SCULK_STONE.defaultBlockState(), 3);
        }
    }
    ...
    private void stretchToFloor(WorldGenLevel level, BlockPos pos, int i, int loop, boolean bottom) {
        ...
        if(bottom) {
            while(!level.getBlockState(blockPos).is(DDBlocks.SCULK_STONE) && !level.getBlockState(blockPos).is(Blocks.DEEPSLATE) && blockPos.getY() > 0) {
                level.setBlock(blockPos, DDBlocks.SCULK_STONE.defaultBlockState(), 3);
                blockPos = blockPos.below();
            }
            return;
        }

        while(!level.getBlockState(blockPos).is(Blocks.SCULK) && !level.getBlockState(blockPos).is(DDBlocks.SCULK_STONE) && !level.getBlockState(blockPos).is(Blocks.DEEPSLATE) && blockPos.getY() < 127) {
            level.setBlock(blockPos, DDBlocks.SCULK_STONE.defaultBlockState(), 3);
