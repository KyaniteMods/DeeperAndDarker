...
public class BloomingStemFeature extends Feature<NoneFeatureConfiguration> {
    ...
    private BlockState stemPlacement(Direction direction, Direction nextDirection, Direction branchDirection) {
        BlockState stem = DDBlocks.BLOOMING_STEM.defaultBlockState().setValue(BloomingStemBlock.DOWN, false).setValue(PipeBlock.PROPERTY_BY_DIRECTION.get(direction.getOpposite()), true).setValue(PipeBlock.PROPERTY_BY_DIRECTION.get(nextDirection), true);
