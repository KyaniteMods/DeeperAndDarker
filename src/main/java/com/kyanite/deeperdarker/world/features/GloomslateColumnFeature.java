...
public class GloomslateColumnFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pContext) {
        ...
        boolean solid = columnHeight * 0.675f - columnHeight * 0.325f < 6;
        for(int i = 1; i < columnHeight + 1; i++) {
            ...
            if(percentageToTop >= 0.325f && percentageToTop <= 0.675f) { // if length of 5 or less, gloomslate
    ...
    private void columnBase(WorldGenLevel level, RandomSource random, BlockPos origin, int columnHeight, boolean bottom) {
        for(int i = 0; i < 4; i++) {
            ...
            placeSection(level, random, origin, baseHeight, i, 1, columnHeight, bottom);
        }

        for(int i = 0; i < 8; i++) {
            int baseHeight = random.nextInt((int) (0.22 * columnHeight), (int) (0.26 * columnHeight) + 1);
            if(i > 3) baseHeight *= 0.67;
            placeSection(level, random, origin, baseHeight, i, 2, columnHeight, bottom);
        }

        if(columnHeight < 19) return;
        for(int i = 0; i < 8; i++) {
            int baseHeight = random.nextInt((int) (0.04 * columnHeight), (int) (0.08 * columnHeight) + 1);
            placeSection(level, random, origin, baseHeight, i, 3, columnHeight, bottom);
        }
    }

    private void placeSection(WorldGenLevel level, RandomSource random, BlockPos pos, int baseHeight, int iteration, int loop, int columnHeight, boolean bottom) {
        float p = random.nextFloat();
        for(int j = 0; j < baseHeight; j++) {
            if(iteration > 3 && columnHeight < 19) return;
