        ...
        for(int i = 0; i <= length; i++) {
            if(level.isEmptyBlock(pos) && !(level.getBlockState(pos).getBlock() instanceof LeavesBlock)) {
                if(i == length || !level.isEmptyBlock(pos.below())) {
                    level.setBlock(pos, DDBlocks.GLOWING_VINES.defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, 17, 25)), 3);
                    break;
                }

                BlockState vines = DDBlocks.GLOWING_VINES_PLANT.defaultBlockState();
