...
public class SculkVinesFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pContext) {
        ...
        if(level.isEmptyBlock(origin)) {
            ...
            if(state.is(Blocks.SCULK) || state.is(DDBlocks.SCULK_STONE) || state.is(DDBlocks.ECHO_LEAVES)) {
        ...
        for(int i = 0; i <= length; i++) {
            if(level.isEmptyBlock(pos)) {
                if(i == length || !level.isEmptyBlock(pos.below())) {
                    level.setBlock(pos, DDBlocks.SCULK_VINES.defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, 17, 25)), 3);
                    break;
                }

                level.setBlock(pos, DDBlocks.SCULK_VINES_PLANT.defaultBlockState(), 3);
            }
