        ...
        for(int i = 1; i <= length; i++) {
            if(level.isEmptyBlock(pos)) {
                if(i == length || !level.isEmptyBlock(pos.above())) {
                    level.setBlock(pos, DDBlocks.SCULK_TENDRILS.defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, 17, 25)), 3);
                    break;
                }

                level.setBlock(pos, DDBlocks.SCULK_TENDRILS_PLANT.defaultBlockState(), 3);
            }
