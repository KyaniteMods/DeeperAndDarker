...
public class Sludge extends Slime {
    ...
    protected void dealDamage(LivingEntity livingEntity) {
        if(this.level() instanceof ServerLevel level && this.isAlive() && this.isWithinMeleeAttackRange(livingEntity) && this.hasLineOfSight(livingEntity)) {
            DamageSource source = this.damageSources().mobAttack(this);
            if(livingEntity.hurtServer(level, source, this.getAttackDamage())) {
    ...
    protected ParticleOptions getParticleType() {
        return new ItemParticleOption(ParticleTypes.ITEM, DDBlocks.BLOOMING_MOSS_BLOCK.asItem());
    }
}
