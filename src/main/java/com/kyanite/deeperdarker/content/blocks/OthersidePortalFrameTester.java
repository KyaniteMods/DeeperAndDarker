...
public class OthersidePortalFrameTester extends VanillaPortalAreaHelper {
    ...
    public PortalFrameTester init(LevelAccessor world, BlockPos blockPos, Direction.Axis axis, Block... foundations) {
        ...
        if (lowerCorner == null) {
            ...
            this.width = this.getSize(axis, DDConfig.HANDLER.getConfig().portalMinWidth, DDConfig.HANDLER.getConfig().portalMaxWidth);
            if (this.width > 0) {
                this.height = this.getSize(Direction.Axis.Y, DDConfig.HANDLER.getConfig().portalMinHeight, DDConfig.HANDLER.getConfig().portalMaxHeight);
    ...
    public boolean isRequestedSize(int attemptWidth, int attemptHeight) {
        return (this.width >= DDConfig.HANDLER.getConfig().portalMinWidth) && (this.height >= DDConfig.HANDLER.getConfig().portalMinHeight);
    }
    ...
    public void createPortal(Level world, BlockPos pos, BlockState frameBlock, Direction.Axis axis) {
        ...
        for (int i = -1; i < DDConfig.HANDLER.getConfig().generatedPortalHeight + 1; i++) {
            world.setBlockAndUpdate(pos.above(i).relative(axis, -1), frameBlock);
            world.setBlockAndUpdate(pos.above(i).relative(axis, DDConfig.HANDLER.getConfig().generatedPortalWidth), frameBlock);
            if (i >= 0) {
                for (int j = 0; j < DDConfig.HANDLER.getConfig().generatedPortalWidth; j++) {
                    fillAirAroundPortal(world, pos.above(i).relative(axis, -1).relative(rotatedAxis, -1));
                    fillAirAroundPortal(world, pos.above(i).relative(axis, -1).relative(rotatedAxis, 1));
                }
            }
        }
        for (int i = -1; i < DDConfig.HANDLER.getConfig().generatedPortalWidth + 1; i++) {
            world.setBlockAndUpdate(pos.above(-1).relative(axis, i), frameBlock);
            world.setBlockAndUpdate(pos.above(DDConfig.HANDLER.getConfig().generatedPortalHeight).relative(axis, i), frameBlock);
        }
        for (int i = 0; i < DDConfig.HANDLER.getConfig().generatedPortalWidth; i++) {
            placeLandingPad(world, pos.above(-1).relative(axis, i).relative(rotatedAxis, -1), frameBlock);
            placeLandingPad(world, pos.above(-1).relative(axis, i).relative(rotatedAxis, 1), frameBlock);
        }
        this.lowerCorner = pos;
        this.width = DDConfig.HANDLER.getConfig().generatedPortalWidth;
        this.height = DDConfig.HANDLER.getConfig().generatedPortalHeight;
        this.axis = axis;
        this.world = world;
