...
public class EchoTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pContext) {
        ...
        if(noSpaceBelow(pContext.level(), pContext.origin(), height + 4)) return false;
        ...
        for(x1 = 0; x1 < 2; x1++) {
            pContext.level().setBlock(new BlockPos(pContext.origin().getX() - x1, pContext.origin().getY() - height, pContext.origin().getZ()), DDBlocks.SCULK_GLEAM.get().defaultBlockState(), Block.UPDATE_ALL);
        }
        ...
        for(x2 = 0; x2 < 2; x2++) {
            pContext.level().setBlock(new BlockPos(pContext.origin().getX() + x2, pContext.origin().getY() - height, pContext.origin().getZ()), DDBlocks.SCULK_GLEAM.get().defaultBlockState(), Block.UPDATE_ALL);
        }
        ...
        for(z1 = 0; z1 < 2; z1++) {
            pContext.level().setBlock(new BlockPos(pContext.origin().getX(), pContext.origin().getY() - height, pContext.origin().getZ() + z1), DDBlocks.SCULK_GLEAM.get().defaultBlockState(), Block.UPDATE_ALL);
        }
        ...
        for(z2 = 0; z2 < 2; z2++) {
            pContext.level().setBlock(new BlockPos(pContext.origin().getX(), pContext.origin().getY() - height, pContext.origin().getZ() - z2), DDBlocks.SCULK_GLEAM.get().defaultBlockState(), Block.UPDATE_ALL);
        }
