...
public class ENLanguageProvider extends LanguageProvider {
    ...
    protected void addTranslations() {
        for(Map.Entry<String, Item> item : DDItems.REGISTERED_ITEMS.entrySet()) if(!(item.getValue() instanceof BlockItem)) add(item.getValue(), convertToName(item.getKey()));
        for(Map.Entry<String, Block> block : DDBlocks.BLOCKS.entrySet()) add(block.getValue(), convertToName(block.getKey()));
        for(Map.Entry<String, EntityType<?>> entity : DDEntities.ENTITIES.entrySet()) add(entity.getValue(), convertToName(entity.getKey()));

        add(OthersideBiomes.OTHERSIDE_DEEPLANDS.registry().toLanguageKey(), convertToName(OthersideBiomes.OTHERSIDE_DEEPLANDS.toString()));
        add(OthersideBiomes.ECHOING_FOREST.registry().toLanguageKey(), convertToName(OthersideBiomes.ECHOING_FOREST.toString()));
